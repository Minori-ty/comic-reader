...
export function LibraryView() {
    ...
    const handleContextMenu = useCallback((e: React.MouseEvent, comic: ComicInfo) => {
        setContextMenu({
            visible: true,
            x: e.clientX,
            y: e.clientY,
    ...
    const handleOpenFileLocation = useCallback(() => {
        invoke('open_file_location', { path: contextMenu.filePath }).catch(e => console.error('open_file_location:', e))
        ...
        <div ref={parentRef} className='library-view'>
            {!libraryPath ? (
                ...
                <div className='library-empty'>
                    ...
                    <p>{t('library.noResultsHint', { query: searchQuery })}</p>
                </div>
            ) : comics.length === 0 ? (
                <div className='library-empty'>
                    <div className='library-empty-icon'>🔍</div>
                    <h2>{t('library.noComics')}</h2>
                    <p>{t('library.noComicsHint')}</p>
                </div>
            ) : (
                <div
                    style={{
            ...
            {contextMenu.visible &&
                createPortal(
                    ...
                    >
                        ...
                        <button className='context-menu-item context-menu-item-danger' onClick={handleTriggerDelete}>
                            {t('library.delete')}
                        </button>
                    </div>,
                    document.body
                ...
                createPortal(
                    <div className='dialog-overlay' onClick={handleCancelDelete}>
                        <div className='dialog' onClick={e => e.stopPropagation()} role='dialog' aria-modal='true'>
                            ...
                            <p className='dialog-message'>{t('library.deleteMsg', { name: deleteDialog.fileName })}</p>
                            ...
                            <div className='dialog-actions'>
                                ...
                                <button className='dialog-btn dialog-btn-danger' onClick={handleConfirmDelete}>
                                    {t('library.delete')}
                                </button>
                            </div>
                        </div>
