            ...
            {scanResult && (
                <div className={`scan-summary ${scanResult.errors.length > 0 ? 'scan-summary-errors' : ''}`}>
                    {scanResult.newComics > 0 && (
                        <span>
                            +{scanResult.newComics} {t('scan.newAdded')}{' '}
                        </span>
                    )}
                    {scanResult.updatedComics > 0 && (
                        <span>
                            ↻{scanResult.updatedComics} {t('scan.updated')}{' '}
                        </span>
                    )}
                    {scanResult.removedComics > 0 && (
                        <span>
                            -{scanResult.removedComics} {t('scan.removed')}{' '}
                        </span>
                    )}
                    {scanResult.skippedComics > 0 && (
                        <span>
                            ✓{scanResult.skippedComics} {t('scan.unchanged')}{' '}
                        </span>
                    )}
                    {scanResult.errors.length > 0 && (
                        <span className='scan-errors'>
                            ⚠ {scanResult.errors.length} {t('scan.errors')}
                        </span>
                    )}
                </div>
            )}
                        ...
                        <div className='share-dialog' onClick={e => e.stopPropagation()}>
                            ...
                            {shareLoading ? (
                                ...
                                <>
                                    ...
                                    <span className={`share-url-copied ${urlCopied ? 'visible' : ''}`}>
                                        {t('share.copied')}
                                    </span>
